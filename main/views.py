...
def attendance_detail(request, team_id):
    ...
    print(team.workers)
    is_att_taken = True if Attendance.objects.filter(date=date.today(), team=team) else False # jamoa uchun davomat olinmagan bo'lsa davomat oling degan tugma chiqarish uchun
...
def attendance_take(request, team_id):
    team = Team.objects.get(id=team_id)
    today = date.today()
    if not Attendance.objects.filter(date=today, team=team):  # -> bir kunda bitta davomatni chiqarish 
